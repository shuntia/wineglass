...
impl Lexer {
    ...
    pub fn get_loc(&mut self) -> tools::Position {
        self.reader.get_loc()
    }
    ...
    fn peek_to_reserved(&mut self) -> Option<String> {
        let mut buf = String::new();
    ...
    pub fn peek_next(&mut self) -> Option<Result<tools::Token, String>> {
        match self.reader.peek() {
            Some(res) => {
                ...
                match c {
                    ...
                    '-' => {
                        if self.reader.peek().unwrap().unwrap_or(' ') == '>' {
                            self.reader.next();
                    ...
                    ':' => {
                        if self.reader.peek().unwrap().unwrap_or(' ') == ':' {
                            return Some(Ok(tools::Token::Simple(tools::SimpleToken::DoubleColon)));
                        }
                        Some(Ok(tools::Token::Simple(tools::SimpleToken::Colon)))
                    }
                    '0'..='9' => {
                        self.buf = c.to_string();
                        while self.reader.peek().unwrap().unwrap().is_numeric()
                            || self.reader.peek().unwrap().unwrap() == '.'
                        {
                    ...
                    c => {
                        if c.is_alphabetic() || c == '_' {
                            self.buf = c.to_string();
                            let next_reserved = match self.peek_to_reserved() {
                                Some(s) => s,
                                None => {
    ...
    pub fn peek_n(&mut self, n: usize) -> Option<Result<tools::Token, String>> {
        for _ in 0..n - 1 {
            match self.next() {
                Some(Ok(t)) => self.tokenbuf.push(t),
...
impl Iterator for Lexer {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if !self.tokenbuf.is_empty() {
            match self.tokenbuf.pop() {
                Some(t) => match t {
                    tools::Token::Unknown(s) => return Some(Err(format!("Unknown Token: {}", s))),
                    _ => return Some(Ok(t)),
                },
                None => return None,
            }
